...
//                  04 June 2019 JE: adding SPI1 port for 2x more Intan RHD2132 (128 chan setup)
// ---------------------------------------
...
// --------------------------------------
// Initialize SPI0 port for amps A and B
// ------------------------------------
...
void SPI_init() {
  ...
  digitalWriteFast(CSpinA, HIGH);

  pinMode(CSpinB, OUTPUT);
  digitalWriteFast(CSpinB, HIGH);
...
void SPI_verify() {
  // --------------------------------------------------------
  // --------- Verify SPI0 interface to INTAN RHD 2132 -------
  // --------------------------------------------------------
  ...
  for (int k = 0; k < 2 ; k++) { //loop twice through, once for A, once for B, set chip select accordingly
    ...
    // print contents of registers we just read
...
// ---------------------------
// SPI1 port for amps C and D
// ---------------------------
void SPI1_init() {


  SERIALNAME->print("Begin and configuring SPI1 bus now...");
  ...
  //SPI.beginTransaction(SPIsettingsIntan);
  SPI1.beginTransaction(SPISettings(SPI_TRANSFER_RATE_DIV, MSBFIRST, SPI_MODE0));  //Based on INTAN requirements, but clock divided down from max to make sure SPI transfer occurs safely

  // Set pin chip select pins high to avoid conficts with the Intan amp we want to address.
  pinMode(CSpinC, OUTPUT);
  digitalWriteFast(CSpinC, HIGH);

  pinMode(CSpinD, OUTPUT);
  digitalWriteFast(CSpinD, HIGH);
  ...
  writeRegister_SPI1(WEAK_MISO_REG_NUM, WEAK_MISO_DATA_BYTE, CSpinC);
  writeRegister_SPI1(WEAK_MISO_REG_NUM, WEAK_MISO_DATA_BYTE, CSpinD);
...
// --------------------------------------------------------
// --------- Verify SPI1 interface to INTAN RHD 2132 ------
// --------------------------------------------------------
void SPI1_verify() {
  ...
  int thisCSpin;
  for (int k = 0; k < 2 ; k++) { //loop twice through, once for C, once for D, set chip select accordingly

    // set chip select pin 
    if (k == 0) {
      thisCSpin = CSpinC;  //for amp C
      SERIALNAME->println("amp C verifying SPI link now.");
    }
    else {
      thisCSpin = CSpinD;  // for ampD
      SERIALNAME->println("amp D verifying SPI link now.");
    }

    // read company designation from registers 40-44
    throwAwayRead =          readRegister_SPI1(40, thisCSpin); //need two extra throw away values due to 2 command lag of SPI data returning.
    throwAwayRead =          readRegister_SPI1(41, thisCSpin); //First 2 responses are non-sensical here, throw them away
    CompanyDesignation[0] =  readRegister_SPI1(42, thisCSpin);
    CompanyDesignation[1] =  readRegister_SPI1(43, thisCSpin);
    CompanyDesignation[2] =  readRegister_SPI1(44, thisCSpin);
    CompanyDesignation[3] =  readRegister_SPI1(40, thisCSpin); //Let's just read regNum 40 again.  readRegister(0) would also be ok, but let's not mess with it.
    CompanyDesignation[4] =  readRegister_SPI1(40, thisCSpin);
...
//  Get amplifier info, die revision, unipolar, etc.
// -----------------------------------------------------
uint16_t SPI1_getAmplInfo() { // returns number of amplifiers on chip
  ...
  for (int k = 0; k < 2 ; k++) { //loop twice through, once for C, once for D, set chip select accordingly
    ...
    if (k == 0) {
      thisCSpin = CSpinC;  //for amp C
      SERIALNAME->println("amp C get info now.");
    }
    else {
      thisCSpin = CSpinD;  // for amp D
      SERIALNAME->println("amp D get info now.");
    }

    //read amplifier info from registers 60-63
    throwAwayRead    =  readRegister_SPI1(60, thisCSpin); // reg60: die revision
    throwAwayRead    =  readRegister_SPI1(61, thisCSpin); // reg61: unipolar/bipolar (should be 1 for us since we use bipolar RHD2132)
    AmplifierInfo[0] =  readRegister_SPI1(62, thisCSpin); // reg62: number of amplifers. should be 32
    AmplifierInfo[1] =  readRegister_SPI1(63, thisCSpin); // reg 63: Intan chip ID, should be 1 for RHD2132
    AmplifierInfo[2] =  readRegister_SPI1(63, thisCSpin); // call 2 more readRegister commands  to get all the data returned
    AmplifierInfo[3] =  readRegister_SPI1(63, thisCSpin);
