...
const unsigned int NUM_BYTES_PER_SCAN = 2 * (2 * NUM_AUX_CMD  + 2 * NUM_AMPS_PER_CHIP) + 4 + 6 + 2; 
                  // 2 Intan chips * 32 adc conversion data (2 bytes each)
                  //  + 2 auxiliary cmd results (2 bytes each) 
                  // + 4 bytes for 32 bit timestamp 
                  //  + 6 (accelerometer xyz)
                  //  +1 sync word
                  // = 2x(64 + 4) + 4 + 6 + 2 = 148 bytes
